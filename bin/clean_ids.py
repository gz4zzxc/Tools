...
def main():
    ...
    if age_input:
        ...
        if (min_val is not None and min_val >= 1000) or (max_val is not None and max_val >= 1000):
            is_year = True
                
    if is_year:
        desc = "不限年份"
        if min_val is not None and max_val is not None:
            if min_val == max_val: desc = f"{min_val}年"
            else: desc = f"{min_val} ~ {max_val} 年出生"
        elif min_val is not None: desc = f"{min_val}年及以后出生"
        elif max_val is not None: desc = f"{max_val}年及以前出生"
    else:
        desc = "不限年龄"
        if min_val is not None and max_val is not None:
            if min_val == max_val: desc = f"{min_val}岁"
            else: desc = f"{min_val} ~ {max_val} 岁"
        elif min_val is not None: desc = f"大于等于 {min_val} 岁"
        elif max_val is not None: desc = f"小于等于 {max_val} 岁"
    ...
    for file_path in txt_files:
        ...
        results = process_file(file_path, target_gender, min_val, max_val, is_year)
        ...
        print("-" * 70)
        print(f"{'编号':<6} {'身份证号':<20} {'性别':<4} {'年龄':<4} {'生日':<10} {'归属地'}")
        print("-" * 70)
        for r in results:
            print(f"{r['index']:<6} {r['id']:<20} {r['gender']:<4} {r['age']:<4} {r['birthday']:<10} {r['location']}")
        print("-" * 70)
